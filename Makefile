...
# disable address sanitizer if we are targeting check-gdb -fsanitize=address,
ifeq ($(findstring check-gdb,$(MAKECMDGOALS)),check-gdb)
FLAGS := $(filter-out  $(FLAGS))
endif
...
check-gdb-%: clean autograder
	sudo echo ""
	DEBUG=1 python3 test/autograder.py "$(patsubst check-gdb-%, %, $@)" & sleep 2 && sudo gdb -p `pgrep autograder`
